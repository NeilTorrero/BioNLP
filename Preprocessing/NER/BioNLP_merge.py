#
# Converting NER dataset to have all of the SOAP columns in one for final NER dataset
#
import pandas as pd
...
df = pd.read_csv(r'Preprocessing/NER/Resources/BioNLP_NER_man_2_with_Ref.csv')
tags = []
tokens = []
...
if separate == 0:
    ...
    df['tags'] = tags
    if skip == 0:
        df.to_csv(r'Resources/BioNLP2_dataset.csv', index=False)
    else:
        df.to_csv(r'Resources/BioNLP2_dataset1.csv', index=False)
else:
    ndf = pd.DataFrame({'tokens': tokens, 'tags': tags})
    ndf.to_csv(r'Resources/BioNLP2_dataset' + str(separate) + str(skip) + '.csv', index=False)