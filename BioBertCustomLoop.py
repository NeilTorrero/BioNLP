...
for i in range(0, 100):
    ...
    mimic = mimic['train'].train_test_split(test_size=0.2, seed=i)#1 21 8 13 42! 64!
    test_valid = mimic['test'].train_test_split(test_size=0.5, seed=i)
    mimic = DatasetDict({
        'train': mimic['train'],
    ...
    results = seqeval.compute(predictions=true_predictions, references=true_labels)
    print('Only MIMIC - Seed ' + str(i))
    print(results)
    log = open("seed_rank.log", "a")
    log.write('Seed ' + str(i) + ': ')
    log.write(str(results) + '\n\n')
    log.close()
    if best_f1 < results['overall_f1']:
        best_f1 = results['overall_f1']
        trainer.save_model('model/f1_end/')
